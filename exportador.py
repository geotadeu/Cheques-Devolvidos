...
class Exportador:
    def exportar(self, tipo, cooperativa, pa, caminho1, caminho2, caminho3):
        ...
        #self.df_cc = self.corrigir_valores_leitura(self.df_cc)
        #self.df_cp = self.corrigir_valores_leitura(self.df_cp)
    ...
    def corrigir_valores_leitura(self, df):
        for index, row in df.iterrows():
            if 'PA Dep' in row and pd.notnull(row['PA Dep']) and len(str(row['PA Dep'])) > 0:
                df.at[index,'PA Dep'] = 'PA' + format(int(''.join(i for i in str(row['PA Dep']) if i.isdigit())), '04')
        return df

    def corrigir_valores_escrita(self, df):
        df['PAC Cliente'] = df['PAC Cliente'].astype(str)
        for index, row in df.iterrows():
            print(row['PAC Cliente'])
            if ('PAC Cliente' in row) and pd.notna(row['PAC Cliente']) and (len(str(row['PAC Cliente']).strip()) > 0):
                s = ''.join(i for i in str(row['PAC Cliente']) if i.isdigit())
                if len(s) > 0:
                    df.at[index,'PAC Cliente'] = 'PA' + format(int(s), '04')
            else:
                print('ignorar' + str(row['PAC Cliente']))
        return df
    ...
    def obter_resumo_ocorrencias(self):
        ...
        #return self.corrigir_valores_escrita(df_todos)
        return df_todos
    ...
    def obter_dados_ocorrencias(self, ocorrencias):
        ...
        df_todos['Data'] = self.data
        df_todos['Data'] = pd.to_datetime(df_todos['Data'], format='%d/%m/%Y %H:%M:%S').apply(lambda x: x.date())
        df_todos = df_todos[['Banco', 'Agencia', 'Cod Dev', 'Ag Dep', 'Conta', 'Ag Dest', 'Nº CH', 'Valor', 'Data', 'PAC Cliente', 'PA Dep', 'Tratativa', 'Ocorrencia']] 
        #return self.corrigir_valores_escrita(df_todos)
        return df_todos
    ...
    def converter_pa(self,value):
        if isinstance(value,int):
            return 'PA' + format(value, '04')
        else:
            if len(value) == 0:
                return -1
    ...
    def tratativa_cc_pa_proprio(self, df):
        df_saida = pd.DataFrame(df.loc[
            (df['Ag Dep'].eq(self.cooperativa)) & 
            (df['Ag Dest'].eq(self.cooperativa)) & 
            (df['PAC Cliente'].eq(self.pac_cliente)) & 
    ...
    def tratativa_cc_pa_enviar(self, df):
        df_saida = pd.DataFrame(df.loc[
            (df['Ag Dep'].eq(self.cooperativa)) &
            (df['Ag Dest'].eq(self.cooperativa)) &
            (df['PAC Cliente'].ne(self.pac_cliente)) & 
            (df['PA Dep'].eq(self.pa))])
    ...
    def tratativa_cc_ic_receber(self, df):
        df_saida = pd.DataFrame(df.loc[
            ...
            (df['PA Dep'].ne(self.pa) | self.busca_geral())])
    ...
    def tratativa_cc_ic_enviar(self, df):
        df_saida = pd.DataFrame(df.loc[
            (df['Ag Dest'].ne(1)) &
            (df['Ag Dest'].ne(self.cooperativa)) &
            (df['PAC Cliente'].ne(self.pac_cliente) | self.busca_geral()) &
